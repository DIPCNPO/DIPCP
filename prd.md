...
### 2.6 编辑功能
- **工具条**：有撤销，反撤销，保存，预览，删除，上传6个图标按钮。工具条和导航条两个区域不随内容滚动，始终保持在顶部。
...
### 2.9 链接功能
- **链接**：显示自己编写的所有已经提交的文件一览表，点击其中一个后弹出模态框，在下拉列表中显示当前作品种所有其他作者还没有建立链接的文章。选则其中一个，可查看过滤掉md标签后的内容，并且自动统计有多少个和当前文件同名的词语。点击确定后向作者发送申请，申请进度保存在links表中。
